...
const items = [
  ...
  { href: "/create", label: "통합 업무 생성", icon: Sparkles },
  { href: "/saved", label: "저장 보관함", icon: FolderOpen },
  { href: "/notice", label: "어르신 일일 알림장", icon: Bell },
  { href: "/newsletter", label: "급여기록/가정통신문", icon: Newspaper },
  { href: "/homepage", label: "센터 소식 게시글", icon: Home },
  { href: "/blog", label: "블로그 홍보글", icon: FileText },
  { href: "/instagram", label: "인스타그램 소식", icon: Instagram },
...
export function Sidebar() {
  ...
  const [email, setEmail] = useState("사회복지사 (게스트)");
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 border-r border-line bg-white px-4 py-5 lg:flex lg:flex-col">
      <Link href="/dashboard" className="flex items-center gap-3 px-2">
        ...
        <div>
          <p className="text-base font-black text-ink">SilverCare AI</p>
          <p className="text-xs font-medium text-muted">사회복지사 업무지원</p>
        </div>
      </Link>
